...
//
// Trajectory (retraction convention: positive = more retracted):
...
//
// acc_ref includes gravity compensation (g) plus the sinusoidal feed-forward term.

class CableSineRefNode : public rclcpp::Node {
 public:
  CableSineRefNode() : Node("cable_sine_ref_node") {
    declare_parameter("freq_hz",      0.5);   // Hz
    declare_parameter("amplitude",    0.1);   // m (retraction convention: positive = retract)
